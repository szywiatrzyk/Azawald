...
Portal::Portal(int x,int y, int z,float rotate)
{
	pos.x = x;
	pos.y = y;
	pos.z = z;
...
bool Portal::load(std::string filename)
{
	struct SFace {
		int v[3];
		int n[3];
...
void Portal::Render() {
	...
	if (chosetex == 1) {
		...
		if (!textureNameclose.empty())
		{
			glEnable(GL_TEXTURE_2D);
			TextureManager::getInstance()->BindTexture(textureNameclose);
